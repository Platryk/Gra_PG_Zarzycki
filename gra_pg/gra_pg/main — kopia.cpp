...
int main()
{
	...
	sf::Window okno_aplikacji(sf::VideoMode(SZEROKOSC_OKNA, WYSOKOSC_OKNA, 32), "Gra PG - Green World");
	...
	while (okno_aplikacji.isOpen()) 
	{
		...
		//const sf::InputStream& = okno_aplikacji.
		...
		for (int i = 0; i < vObiekty.size() - 1; i++) {
			if (vObiekty[kolejnosc_wyswietlania_obiektow_z_vObiekty[i]]->podajPredkosc() != 0 || vObiekty[kolejnosc_wyswietlania_obiektow_z_vObiekty[i + 1]]->podajPredkosc() != 0) {
